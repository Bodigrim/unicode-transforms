...
genRangeCheck :: (Ord a, Show a) => String -> [a] -> String
...
genByteMap :: String -> [(Char, Int)] -> String
genByteMap funcName ordList = unlines
    [ funcName <> " :: Char -> Int"
    , funcName <> " = \\c -> let !(I# n#) = ord c in if " ++ genRangeCheck "c" (map fst ordList) ++ " then I# (word2Int# (indexWord8OffAddr# byteMap# n#)) else 0"
    , "  where"
    , "    byteMap# = " ++ show (map chr (positionsToByteMap ordList)) ++ "#"
    ]

positionsToByteMap :: [(Char, Int)] -> [Int]
positionsToByteMap = go (chr 0)
    where
        go _ [] = []
        go i xxs@((x, w) : xs)
            | i < x     = 0 : go (succ i) xxs
            | otherwise = fromIntegral w : go (succ i) xs
...
genCombiningClass props file = unlines
            ...
            , ""
            , "import Data.Char (ord)"
            , "import GHC.Exts (Int(..), indexWord8OffAddr#, word2Int#)"
            , ""
            , genByteMap "getCombiningClass" ccmap
            , ""
            , "{-# INLINE isCombining #-}"
            , "isCombining :: Char -> Bool"
            , "isCombining c = getCombiningClass c /= 0"
            ]
    where
        ccmap = dropWhile    ((== 0) . snd)
              $ dropWhileEnd ((== 0) . snd)
